...
const DoctorVideoCallPage = () => {
  ...
  useEffect(() => {
    const initializeAndCall = async () => {
      ...
      try {
        ...
        setError('Failed to initialize the video call. Please try again.');
        setLoading(false);
      }
    };
    initializeAndCall();
  ...
  const handleCall = async () => {
    try {
      ...
      await TUICallKitServer.call({ userID: calleeUserID, type: TUICallType.VIDEO_CALL });
    } catch (error) {
  ...
  return (
    <Container maxWidth="sm" className="video-call-container">
      <Paper elevation={3} className="video-call-paper">
        ...
        {loading ? (
          ...
          <>
            <Typography variant="body1" gutterBottom>
              You are about to start a video call with {patientData?.name || "your patient"}.
            </Typography>
